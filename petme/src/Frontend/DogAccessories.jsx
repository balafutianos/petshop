// src/Frontend/DogAccessories.jsx
  ...
  const greekMenuItems = [
    { label: "Αρχική", isHome: true, submenu: [] },
    {
      ...
      submenu: [
        ...
        { label: "Αξεσουάρ/Παιχνίδια", href: "/dogs/dogaccessories", active: true },
      ],
      activeParent: true,
    },
    {
    ...
    <>
      ...
      >
        <nav className="breadcrumbs" aria-label="Θέση στη σελίδα" style={{ display: "flex", gap: 8, marginBottom: 14 }}>
          ...
          <span>Σκύλος</span>
          <span aria-hidden="true">›</span>
          <span aria-current="page">Αξεσουάρ/Παιχνίδια</span>
        </nav>

        <section
          className="dog-hero"
          role="img"
          aria-label="Παιχνίδια και Αξεσουάρ Σκύλου"
          style={{
            background:
        ...
        >
          <div>
            <h1>Παιχνίδια & Αξεσουάρ Σκύλου</h1>
            <p style={{ opacity: 0.95, marginTop: 6 }}>Λουριά, παιχνίδια, μπολάκια και πολλά ακόμη.</p>
          </div>
        </section>
            ...
            {dogproducts.map((p) => (
              <li key={p.id} className="product-card">
                <a
                  className="card-link"
                  href={`#/product/${p.id}`}
                  onClick={(e) => e.preventDefault()} // μην αλλάζει σελίδα όταν πατάς τα icon-buttons
                >
                  ...
                  <div className="card-body">
                    ...
                    {/* Δείξε ό,τι πεδία υπάρχουν (brand/type/lifeStage μπορεί να λείπουν στα accessories) */}
                    {(p.brand || p.type || p.lifeStage) && (
                      <p className="meta">
